...
router.post("/register", async (req, res) => {
    try {
        ...
        //req.body.password = await hashService.generateHash(newUser.password);
...
router.post("/users/login", async (req, res) => {
    try {
        ...
        console.log("current user deets= ", req.body);
        const currentUser = await userServiceModel.getUserByEmail(req.body.email);
        console.log("axios userFound = ", currentUser);
        if (!currentUser) {
...
router.get("/users/", loggedInCheck, checkCredentials(true), async (req, res) => {
    try {
...
router.delete("/:id", loggedInCheck, checkCredentials(true), async (req, res) => {
    try {
        await validateId({ id: req.params.id });
